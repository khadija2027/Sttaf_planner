...
creneaux = ["AM", "PM", "GA"]
annee, mois = 2025, 6
jours = [date(annee, mois, i) for i in range(1, 31)]
jours_sem = [j for j in jours if j.weekday() < 5]  
# Spécialités par activitégit branch -M main
specialites = {
...
for m in medecins:
    for (j, c, a), required in besoins.items():
        if isinstance(j, str):
            j_date_part = j.split()[0]
            j_date = datetime.strptime(j_date_part, "%Y_%m_%d").date()
        else:
            j_date = j
        if m == "Bensouda Anas" and date(2025, 6, 1) <= j_date <= date(2025, 6, 25):
            disponibilites[(m, j, c, a)] = 0
        else:
            disponibilites[(m, j, c, a)] = 1
...
pivot.to_csv("affectations_mois.csv")
...
pivot_rempl.to_csv("remplacants_mois.csv")
...
plt.title("Charge de travail par médecin pendant juin 2025")
plt.legend()
plt.tight_layout()
